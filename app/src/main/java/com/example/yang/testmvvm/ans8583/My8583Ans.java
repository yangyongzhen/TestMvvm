package com.example.yang.testmvvm.ans8583;

import android.util.Log;

import java.util.Arrays;

import static com.example.yang.testmvvm.utils.DesUtil.DES_decrypt_3;
import static com.example.yang.testmvvm.utils.DesUtil.DES_encrypt;
import static com.example.yang.testmvvm.utils.DesUtil.DES_encrypt_3;
...
public class My8583Ans extends Easy8583Ans {
    ...
    byte[] getFieldData55(byte[] src ,int money,String date){
        ...
        //String tags[] = {"9f26","9f10","9f37","9f36","82","9f63"};
        if(date.length() != 6) {
            return null;
        }
        TlvAns tlvAns = new TlvAns();
        //开始解析:
        tlvAns.pbocTlvAns(src,src.length);
        cardSnNumber ="";
        cardValiteDate="";
        field35Data ="";
        upBinNumber="";
        for(TlvAns.Tag tag : tlvAns.tags){
            if(tag.tag.equals("5f34")){
                cardSnNumber= tag.value;
            }
            if(tag.tag.equals("57")){
                field35Data = tag.value;
                String tmp = tag.value;
                upBinNumber= tmp.substring(0,tmp.indexOf('d'));
                tmp = tag.value;
                cardValiteDate = tmp.substring(20,20+4);
            }
            if(tag.tag.equals("9f26")){
                out.append(tag.tag+tag.length+tag.value);
            }
            if(tag.tag.equals("9f37")){
                out.append(tag.tag+tag.length+tag.value);
            }
            if(tag.tag.equals("9f10")){
                out.append(tag.tag+tag.length+tag.value);
            }
            if(tag.tag.equals("9f36")){
                out.append(tag.tag+tag.length+tag.value);
            }
            if(tag.tag.equals("9f63")){
                out.append(tag.tag+tag.length+tag.value);
            }
            if(tag.tag.equals("82")){
                out.append(tag.tag+tag.length+tag.value);
            }
        }
        out.append("9f270180");
        out.append("95050000000000");
        out.append("9c0100");
        out.append("5f2a020156");
        out.append("9f1a020156");
        out.append("9f0306000000000000");
        out.append("9f3303e0e1c8");
        out.append("9f3403020000");
        out.append("9f350122");
        out.append("9f1e083030303030303030");
        out.append("9f0902008c");

        out.append("9f4104");//9F41,终端维护的交易计数器
        String str = String.format("%08d",posSn);
        out.append(str);

        out.append("9f0206");//交易金额
        String str1 = String.format("%012d",money);
        out.append(str1);

        out.append("9a03");//交易日期
        out.append(date);
        //out.append("8408A000000333010101");
        return hexStringToBytes(out.toString());
    ...
    public static void main(String[] args) {
        ...
        String str = "";
        byte[] ans = hexStringToBytes(str);
        myans.ans8583Fields(ans,ans.length,myans.fieldsRecv);
        System.out.println(myans.getFields(myans.fieldsRecv));
    }
}
